...
export const ProductDetails = () => {
  const { isProductDetailsActive, setProductDetailsActive, productDetailsSelected } = useProductDetailsStore()
  ...
  return (
    <aside className={`${styles.aside} ${isProductDetailsActive ? styles.isActive : ''} font-quicksand py-5 lg:py-7`}>
      <div className='flex gap-2 text-primary cursor-pointer' onClick={handleCloseAside}>
      ...
      <div className='overflow-y-auto'>
        <figure className={`${styles.figure}`}>
          <Image src={urlImage as string} width='300' height='219' alt={productDetailsSelected.name} />
        </figure>
        <span className='block'>name</span>
        <h3 className='font-medium mt-3 mb-8 text-2xl'>{productDetailsSelected.name}</h3>
      ...
      <div className='flex gap-5 xs:gap-10 items-center justify-center text-base font-bold mt-auto'>
        ...
        <button className='bg-primary text-white rounded-xl px-6 py-4' onClick={handleAddProducToShoppinglist}>
          Add to list
        </button>
      </div>
    </aside>

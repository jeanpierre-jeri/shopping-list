...
export default async function shoppingListByIdHandler(req: NextApiRequest, res: NextApiResponse) {
  const { id = '' } = req.query
  const { products = [], status = 'incomplete' } = req.body as ShoppingListUpdate
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      return shoppingList === null ? res.status(404).json({ msg: 'No shopping list found' }) : res.json(shoppingList)
    } catch (error) {
      console.error(error)

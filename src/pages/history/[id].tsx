...
import { BackArrowIcon, DateIcon } from '@/components/atoms'
    ...
    <>
      {
        isLoading
          ...
          : <div className='px-5 lg:px-20 py-10'>
            ...
            <h1 className='font-quicksand font-bold text-2xl text-[#34333A] mt-9'>{data?.name}</h1>
            <div className='text-gray flex gap-3 font-quicksand text-xs font-medium items-center mt-5'>
              <DateIcon />
              <p>
                {resetDate(data?.createdAt as string)}
              </p>
            </div>
            <div className='flex flex-wrap gap-5 mt-14'>
              {
                data?.products?.length === 0
                  ...
                  : data?.products?.map(({ product, quantity, productId }) => (
                    <div key={productId} className='bg-white shadow-lg py-4 px-4 rounded-xl flex justify-between w-[182px] items-center'>
